...
// Helper to render with auth layout (no header/footer)
function renderAuth(res, page, locals = {}) {
  res.render('pages/' + page, { layout: 'auth', ...locals }, function (err, body) {
    if (err) return res.status(500).send(err.message);
    res.render('layouts/auth', { body, ...locals });
  });
}
...
router.get('/login', function (req, res) {
  renderAuth(res, 'login', { title: '登入', pageScript: 'login' });
});
