...
function initializeDatabase() {
  ...
  // Preserve historical totals while adding shipping snapshots.
  db.transaction(() => {
    const columns = new Set(db.prepare('PRAGMA table_info(orders)').all().map(c => c.name));
    const additions = {
      subtotal: 'INTEGER NOT NULL DEFAULT 0', shipping_fee: 'INTEGER NOT NULL DEFAULT 0',
      shipping_method: "TEXT NOT NULL DEFAULT 'home'",
      is_remote: 'INTEGER NOT NULL DEFAULT 0', is_express: 'INTEGER NOT NULL DEFAULT 0',
    };
    for (const [name, definition] of Object.entries(additions)) {
      if (!columns.has(name)) db.exec('ALTER TABLE orders ADD COLUMN ' + name + ' ' + definition);
    }
    if (!columns.has('subtotal')) db.exec('UPDATE orders SET subtotal = total_amount');
  })();
