...
createApp({
  setup() {
    ...
    const cartItems = ref([]);
    const form = ref({ recipientName: '', recipientEmail: '', recipientAddress: '', recipientPhone: '', note: '', shippingMethod: 'home', isRemote: false, isExpress: false });
    const errors = ref({});
    ...
    const shipping = computed(() => Shipping.calculateShipping({ ...form.value, subtotal: cartTotal.value }));
    ...
    return { loading, submitting, cartItems, form, errors, cartTotal, shipping, submitOrder };
  }
}).mount('#app');
